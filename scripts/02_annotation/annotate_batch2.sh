#!/bin/bash
#SBATCH --job-name=bakta_batch2
#SBATCH --output=bakta_batch2_%j.out
#SBATCH --error=bakta_batch2_%j.err
...
OUTDIR="/scratch/users/k22017808/kp_annotations/batch2"
mkdir -p $OUTDIR

count=0
for genome in /scratch/users/k22017808/kp_liver_project/genomes_fasta/all_genomes/*.fna; do
    if [ $count -lt 10 ]; then count=$((count+1)); continue; fi
