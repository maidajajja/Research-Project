#!/bin/bash
#SBATCH --job-name=bakta_fasta
#SBATCH --output=bakta_fasta_%j.out
#SBATCH --error=bakta_fasta_%j.err
...
OUTDIR="/scratch/users/k22017808/kp_annotations/fasta_genomes"
mkdir -p $OUTDIR

count=0
for genome in /scratch/users/k22017808/kp_liver_project/genomes_fasta/all_genomes/*.fasta; do
    count=$((count+1))
    basename=$(basename $genome .fasta)
    echo "Processing: $basename ($count/82)"
