#!/bin/bash
#SBATCH --job-name=bakta_batch3
#SBATCH --output=bakta_batch3_%j.out
#SBATCH --error=bakta_batch3_%j.err
...
OUTDIR="/scratch/users/k22017808/kp_annotations/batch3"
mkdir -p $OUTDIR

count=0
for genome in /scratch/users/k22017808/kp_liver_project/genomes_fasta/all_genomes/*.fna; do
    if [ $count -lt 60 ]; then count=$((count+1)); continue; fi
    if [ $count -ge 110 ]; then break; fi
