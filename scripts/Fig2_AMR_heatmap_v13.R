...
# Check ST29 acquired genes
cat("ST29 acquired genes per isolate:\n")
print(total_acquired[split_vec == "ST29"])

# Paul Tol muted palette - fixed Macrolide to avoid clash with Host Health
class_colours <- c(
  "Aminoglycoside" = "#44AA99",  # teal
  "Beta-lactam"    = "#6699CC",  # medium blue
  "Macrolide"      = "#997700",  # dark gold — distinct from Host Health amber
  "Quinolone"      = "#CC6677",  # muted rose
  "Sulfonamide"    = "#AA4499",  # muted purple
  "Tetracycline"   = "#882255",  # dark magenta
  "Intrinsic*"     = "#BBBBBB"   # grey
)

prev_cols <- sapply(as.character(col_split), function(x) class_colours[x])

# Top annotation — prevalence bar separate from right annotation
# Fix overlap by not putting right annotation label near top
top_ann <- HeatmapAnnotation(
  ...
  gap = unit(3, "mm"),
  show_legend = c(`Prevalence (%)` = FALSE, `AMR Class` = TRUE),
  annotation_legend_param = list(
    `AMR Class` = list(
      ...
      at = class_levels,
      labels = c("Aminoglycoside","Beta-lactam",
                 "Macrolide","Quinolone","Sulfonamide",
                 "Tetracycline","Intrinsic (*chromosomal)"))))

# Right annotation — moved label to bottom to avoid top-right overlap
...
# Host Health — fixed amber to avoid Macrolide clash
# Using a distinct purple-grey for Non-liver
hh_cols <- c(
  "Liver abscess"            = "#994455",  # dark rose
  "Liver transplant"         = "#4477AA",  # medium blue
  "Other liver disease"      = "#228833",  # medium green
  "Non-liver or unspecified" = "#887799",  # muted purple-grey — distinct from Macrolide gold
...
png(file.path(OUT,"Fig2_AMR_heatmap_v13.png"),
    width=18, height=fig_height, units="in", res=300)
draw(ht, merge_legend=TRUE,
     padding=unit(c(8,10,8,15),"mm"),
     background="white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v13.pdf"), width=18, height=fig_height)
draw(ht, merge_legend=TRUE, padding=unit(c(8,10,8,15),"mm"))
dev.off()

message(paste0("Fig2 v13 saved - ", n_rows, " isolates — final version"))
