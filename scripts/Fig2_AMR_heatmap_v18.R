...
n_rows <- nrow(mat)
fig_height <- max(20, n_rows * 0.09 + 10)
...
ht <- Heatmap(
  ...
  column_names_gp = gpar(fontsize=24, fontface="italic"),
  column_names_rot = 90,
...
png(file.path(OUT,"Fig2_AMR_heatmap_v18.png"),
    width=28, height=fig_height, units="in", res=300)
draw(ht,
     merge_legend = TRUE,
     annotation_legend_list = list(gene_annotation_legend),
     padding = unit(c(60, 30, 90, 50), "mm"),
     background = "white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v18.pdf"),
    width=28, height=fig_height)
draw(ht,
     merge_legend = TRUE,
     annotation_legend_list = list(gene_annotation_legend),
     padding = unit(c(60, 30, 90, 50), "mm"))
...
message(paste0("Fig2 v18 saved - ", n_rows, " isolates"))
