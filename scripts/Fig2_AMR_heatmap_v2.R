...
meta$Source <- dplyr::recode(meta$Source,
  ...
  "Liver abscess puncture fluid"="Drainage/Pus",
  "abscess drainage"="Drainage/Pus",
  "liver drainage"="Drainage/Pus",
  "liver"="Liver",
  "blood"="Blood",
  "Blood"="Blood"
)
...
# ST ordering: ascending isolate count (fewest at top, most at bottom)
st_counts <- sort(table(ann$ST[ann$ST!="Unknown"]), decreasing=FALSE)
top_sts <- tail(names(st_counts), 7)  # 7 most common
top_sts <- rev(names(sort(table(ann$ST[ann$ST %in% names(st_counts)]),
                          decreasing=FALSE))[1:7])
# actually: order ascending so fewest isolates first
# order STs by mean gene count per isolate ascending
st_mean <- tapply(rowSums(mat[match(ann$Sample, rownames(mat)),]), ann$ST, mean, na.rm=TRUE)
st_mean <- st_mean[names(st_mean) != "Unknown"]
st_order <- names(sort(st_mean, decreasing=FALSE))
top_sts <- tail(st_order, 7)  # top 7 by count
top_sts_asc <- head(st_order[st_order %in% top_sts], 7)  # ascending order

wong_pal <- c("#E69F00","#56B4E9","#009E73","#F0E442","#0072B2","#D55E00","#CC79A7")
st_cols <- setNames(c(wong_pal,"grey80","grey90"),
...
# within each ST: ascending gene count (stepped pattern, fewest genes at top)
ann$gene_count <- rowSums(mat[match(ann$Sample, rownames(mat)),])
ann <- ann %>% arrange(factor(ST_group, levels=c(top_sts_asc,"Other","Unknown")),
                       gene_count)
mat <- mat[match(ann$Sample, rownames(mat)),]
ann$ST_split <- factor(ann$ST_group,
                       levels=c(top_sts_asc,"Other","Unknown"))
...
message("Fig2 v2 saved at 600 DPI")
# ordering fix applied in v3
