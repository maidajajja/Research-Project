...
# ALL 229 isolates - ST order by mean AMR gene count descending
...
# Paul Tol muted palette for AMR classes
# Carefully chosen to not clash with ST colours below
class_colours <- c(
  "Aminoglycoside" = "#44AA99",  # teal
  "Beta-lactam"    = "#BBCCEE",  # pale blue — lighter to avoid ST65 clash
...
# ST colours - carefully chosen to not clash with AMR class colours
# ST65 changed from blue to avoid Beta-lactam clash
# ST23 and ST86 made clearly distinct
st_cols <- setNames(c(
  "#332288",  # ST258 - dark indigo
  "#225522",  # ST11  - dark forest green
  "#77AADD",  # ST512 - medium blue
  "#EE8866",  # ST65  - muted orange-red (distinct from all class colours)
  "#774411",  # ST23  - dark brown
  "#FFAABB",  # ST86  - light pink (clearly distinct from ST23 brown)
  "#BBBBBB",  # ST29  - light grey
  "#EEEEEE"   # Other - very light grey
), st_levels)

# Host Health colours - distinct from both ST and AMR class palettes
hh_cols <- c(
  "Liver abscess"            = "#994455",  # dark rose
  "Liver transplant"         = "#4477AA",  # medium blue
  "Other liver disease"      = "#228833",  # medium green
  "Non-liver or unspecified" = "#CCBB44",  # muted yellow-green
...
png(file.path(OUT,"Fig2_AMR_heatmap_v5.png"),
    width=16, height=20, units="in", res=300)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"), background="white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v5.pdf"), width=16, height=20)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"))
dev.off()

message("Fig2 v5 saved - all 229 isolates, fixed colours, 2mm row height")
