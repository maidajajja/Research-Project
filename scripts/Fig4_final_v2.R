...
# extract base accession from plasmid Sample e.g. GCA_000338255.1_KP1... -> GCA_000338255.1
...
meta$GenomeID_num <- as.numeric(meta[["Genome ID"]])
...
meta <- meta[, c("GenomeID_num","Accession","Country")]
...
kleb$GenomeID_num <- as.numeric(kleb$strain)
st_map <- data.frame(GenomeID_num=kleb$GenomeID_num, ST=kleb$ST,
                     stringsAsFactors=FALSE)

# join meta with ST via GenomeID
meta2 <- left_join(meta, st_map, by="GenomeID_num")

# join plasmid with meta2 via Accession
df <- left_join(plasmid, meta2, by="Accession")
...
message("Top STs in plasmid data:")
