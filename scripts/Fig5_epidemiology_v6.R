...
# ── Panel A: Bubble plot — only years with data on x-axis ───────────────────
st_year <- df %>%
...
ggsave(file.path(OUT,"Fig5a_ST_over_time.png"), figA, width=8, height=6, dpi=300, bg="white")
ggsave(file.path(OUT,"Fig5a_ST_over_time.pdf"), figA, width=8, height=6)
...
# reorder columns: Country, then STs in order, then Total
st_cols_order <- st_labels[st_labels %in% colnames(tbl)]
tbl <- tbl[, c("Country", st_cols_order, "Total")]

png(file.path(OUT,"Fig5b_ST_country_table.png"), width=1400, height=400, res=150)
grid.newpage()
grid.rect(gp=gpar(fill="white", col="white"))
tt <- tableGrob(tbl, rows=NULL,
  theme=ttheme_minimal(
    core=list(fg_params=list(fontsize=10, fontface="plain"),
              bg_params=list(fill=c("grey97","white"), col=NA)),
    colhead=list(fg_params=list(fontsize=11, fontface="bold"),
                 bg_params=list(fill="grey20", col=NA, fontcolor="white")),
    rowhead=list(fg_params=list(fontsize=10))
  ))
# colour header text white
for(i in seq_len(ncol(tbl)+1)){
  tt$grobs[[i]]$gp$col <- "white"
}
grid.draw(tt)
dev.off()

message("Fig5 v6 saved")
