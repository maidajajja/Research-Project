...
# clean ST — strip locus variants, strip existing ST prefix
kleb$ST <- gsub("-[0-9]+LV$","", kleb[[st_col]])
kleb$ST <- gsub("-.*","", kleb$ST)
kleb$ST <- gsub("^ST","", kleb$ST)  # remove ST prefix if present
...
top_sts <- names(sort(table(df$ST[!df$ST %in% c("Unknown","")]),
                      decreasing=TRUE))[1:7]
