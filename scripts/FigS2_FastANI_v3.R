...
# Convert to distance (100 - ANI) so similar = low value = dark purple
dist_mat <- 100 - mat
...
ann_df <- data.frame(Sample=rownames(dist_mat), stringsAsFactors=FALSE) %>%
...
# Viridis colour scale — dark purple (similar) to yellow (different)
viridis_cols <- colorRampPalette(c("#440154","#482878","#3E4989","#31688E",
                                    "#26828E","#1F9E89","#35B779","#6DCD59",
                                    "#B4DE2C","#FDE725"))(100)

png(file.path(OUT,"FigS2_FastANI_heatmap_v3.png"), width=14, height=12, units="in", res=300, bg="white")
pheatmap(dist_mat,
         color=viridis_cols,
         ...
         border_color=NA,
         main="K. pneumoniae Pairwise Genomic Distance Matrix (n = 234)",
         fontsize=11,
         annotation_legend=TRUE)
dev.off()

pdf(file.path(OUT,"FigS2_FastANI_heatmap_v3.pdf"), width=14, height=12)
pheatmap(dist_mat,
         color=viridis_cols,
         annotation_row=ann_row,
         annotation_col=ann_row,
