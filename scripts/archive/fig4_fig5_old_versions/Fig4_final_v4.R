...
# join and deduplicate: one row per sample per plasmid type
...
# Top 15 plasmid replicons by number of isolates carrying them
top_plasmids <- names(sort(table(df$Accession[df$Plasmid %in% unique(df$Plasmid)]),
                           decreasing=TRUE))
# actually top by plasmid frequency
