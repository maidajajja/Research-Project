...
# Define gene categories based on biology and AMRFinder method
...
cat("Point mutation genes:", point_mutation_genes, "\n")
cat("Intrinsic genes:", intrinsic_kp, "\n")
...
# 7 key STs ordered by mean acquired gene count
top_sts <- c("ST258","ST11","ST512","ST65","ST23","ST86","ST29")
pa2 <- pa2 %>% filter(ST %in% top_sts)
pa2$ST_group <- factor(pa2$ST, levels=top_sts)

acquired_in_mat <- setdiff(top_genes, c(intrinsic_kp))[setdiff(top_genes, c(intrinsic_kp)) %in% colnames(pa2)]
...
# ST labels with sample sizes
st_counts <- table(split_vec)
st_labels <- paste0(top_sts, "\n(n=", st_counts[top_sts], ")")
names(st_labels) <- top_sts

# Gene ordering: acquired by class, intrinsic at right
class_levels <- c("Aminoglycoside","Beta-lactam","Fosfomycin","Macrolide",
...
# Gene labels with symbols
gene_labels <- ifelse(gene_order %in% point_mutation_genes,
                      paste0(gene_order, "\u2020"),  # dagger for point mutations
               ifelse(gene_order %in% intrinsic_kp,
                      paste0(gene_order, "*"),        # asterisk for intrinsic
                      gene_order))

# Prevalence per gene (% of all 229 isolates)
gene_prev <- colSums(mat) / nrow(mat) * 100

# Total acquired genes per isolate (for right bar)
...
# Top annotation: AMR class bar + prevalence bar
top_ann <- HeatmapAnnotation(
  `Prevalence (%)` = anno_barplot(
    gene_prev,
    gp = gpar(fill = col_split %>% as.character() %>%
      sapply(function(x) class_colours[x]),
              col = NA),
    height = unit(20, "mm"),
...
# Right annotation: total acquired genes bar
right_ann <- rowAnnotation(
  `Acquired\ngenes` = anno_barplot(
    total_acquired,
    gp = gpar(fill="#5b5b5b", col=NA),
    width = unit(20, "mm"),
    axis_param = list(side="bottom", gp=gpar(fontsize=7))),
...
png(file.path(OUT,"Fig2_AMR_heatmap_v10.png"),
    width=18, height=fig_height, units="in", res=300)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"), background="white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v10.pdf"), width=18, height=fig_height)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"))
dev.off()

message(paste0("Fig2 v10 saved - ", n_rows, " isolates, prevalence bars, acquired gene counts, point mutation markers"))
