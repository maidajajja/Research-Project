...
# Gene labels - † for point mutations, * for intrinsic
...
# Top annotation — prevalence bar with proper spacing, no overlap
top_ann <- HeatmapAnnotation(
  ...
  annotation_legend_param = list(
    `AMR Class` = list(
      title = "AMR Class\n(*intrinsic, \u2020point mutation)",
      title_gp = gpar(fontsize=9, fontface="bold"),
      labels_gp = gpar(fontsize=8))))

# Right annotation — acquired genes bar, no axis label overlap
...
png(file.path(OUT,"Fig2_AMR_heatmap_v12.png"),
    width=18, height=fig_height, units="in", res=300)
draw(ht, merge_legend=TRUE, padding=unit(c(5,10,5,15),"mm"), background="white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v12.pdf"), width=18, height=fig_height)
draw(ht, merge_legend=TRUE, padding=unit(c(5,10,5,15),"mm"))
dev.off()

message(paste0("Fig2 v12 saved - ", n_rows, " isolates"))
