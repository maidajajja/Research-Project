...
# Remove "Other" ST group - focus on 7 key lineages only
...
# AMR class colours for bottom annotation bar - Paul Tol muted
class_colours <- c(
...
# Bottom annotation - AMR class colour bar like reference figure
...
st_cols <- setNames(c(
  ...
  "#CC99BB",  # ST86  - muted mauve (distinct from ST23)
  "#999999"   # ST29  - medium grey (distinct from both)
), top_sts)
...
# Left row annotation - ST and Host Health
...
ht <- Heatmap(
  ...
  # Bottom annotation - AMR class colour bar
  bottom_annotation = bot_ann,
  # Row split by ST
  row_split = split_vec,
  cluster_rows = FALSE,
  cluster_row_slices = FALSE,
  # Column split by AMR class with gaps
  column_split = col_split,
  column_gap = unit(3, "mm"),
  cluster_columns = FALSE,
  cluster_column_slices = FALSE,
  show_row_names = FALSE,
  # Gene names at bottom below colour bar
...
png(file.path(OUT,"Fig2_AMR_heatmap_v7.png"),
    width=16, height=16, units="in", res=300)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"), background="white")
dev.off()

pdf(file.path(OUT,"Fig2_AMR_heatmap_v7.pdf"), width=16, height=16)
draw(ht, merge_legend=TRUE, padding=unit(c(5,5,5,15),"mm"))
dev.off()

message("Fig2 v7 saved - bottom annotation, Other removed, 7 key STs only")
