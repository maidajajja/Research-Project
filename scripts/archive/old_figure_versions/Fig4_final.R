...
# extract the bare SRA-style accession for the new fallback match
# (plasmid Sample values for SRA-derived genomes look like
# "SRR26938680_assembled" - strip the suffix to get the raw accession)
...
# NOTE: ST now sourced from genomes.csv's own MLST column rather than
# re-deriving it elsewhere. Cross-validated against Kleborate's
# independently-computed ST: 190/190 genomes agree exactly (see
# validate_csv_mlst_v2.R). This column has 212/235 coverage, better
# than the standalone mlst tool's 82/235.
...
# NEW: SRA Accession fallback lookup table, keyed the same way the
# plasmid Sample's stripped accession will be matched against
...
# First pass: match via GCA/numeric accession as before
df <- left_join(plasmid, meta_lookup, by="Accession") %>%
  distinct(Accession, Plasmid, .keep_all=TRUE)

# Second pass: for rows still unmatched, try the SRA Accession fallback
unmatched_rows <- is.na(df$ST)
if (any(unmatched_rows)) {
  sra_match <- meta_sra[match(df$Accession_SRA_guess[unmatched_rows], meta_sra$Accession_SRA), ]
...
message("Fig4 v6 saved - added SRA Accession fallback join")
